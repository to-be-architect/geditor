            ...
            if (request.getContentLength() < Constants.MAX_REQUEST_SIZE) {
                ...
                if (!format.equals("xml")) {
                    if (xml == null || xml.length() == 0) {
                        writeScript(writer,
                                "window.parent.alert(JSON.stringify({message:window.parent.mxResources.get('invalidOrMissingFile')}));");
                    } else {
                        ...
                        writeScript(writer,
                                "try{window.parent.setCurrentXml(decodeURIComponent('"
                                        ...
                                        + "'));}catch(e){window.parent.alert(JSON.stringify({message:window.parent.mxResources.get('notAUtf8File')}));}");
                    }
                } else {
                    writer.println(xml);
                }
                ...
                writeScript(writer,
                        "window.parent.alert(JSON.stringify(window.parent.mxResources.get('drawingTooLarge')));");
            }
